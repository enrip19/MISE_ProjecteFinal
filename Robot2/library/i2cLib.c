...
void I2C_send (uint8_t addr, uint8_t *buffer, uint8_t n_dades ){
    UCB1I2CSA = addr;                                                   //Coloquem lï¿½adreï¿½a de slave
    PTxData = buffer;                                                   //adrec,a del bloc de dades a transmetre
    TXByteCtr = n_dades;                                                //carreguem el nï¿½mero de dades a transmetre;
    while(EUSCI_B1 ->CTLW0 & EUSCI_B_CTLW0_TXSTP);                      //Ens assegurem que s'ha enviat la condiciÃ³ de stop
    ...
    while(EUSCI_B1 -> CTLW0 & UCTXSTP);                                 //Ens assegurem que s'ha enviat la condiciÃ³ de stop
}
...
void I2C_receive(uint8_t addr, uint8_t *buffer, uint8_t n_dades)
{
    ...
    UCB1I2CSA = addr;                                                   //Coloquem lâ€™adreÃ§a de slave
    UCB1CTL1 &= ~UCTR;                                                  //I2C en mode RecepciÃ³
    while (UCB1CTL1 & UCTXSTP);                                         //Ens assegurem que el bus estÃ  en stop
    UCB1CTL1 |= UCTXSTT;                                                //I2C start condition en recepciÃ³
    /* POSEM A DORMIR EL SISTEMA EN MODE EQUIVALENT A LPM0 + IE */
    SCB->SCR |= SCB_SCR_SLEEPONEXIT_Msk;                                //Configurem el mode LPM0 (CPU apagada, nomes funcionen interrupcions)
    __DSB();                                                            //Acabem totes les transferencies (en aquest cas no en tenim)
    __sleep();                                                          //Posem el sistema a dormir (activem LPM0)
    ...
    case 0x16:
        if(RXByteCtr){
          *PRxData++ = EUSCI_B1 -> RXBUF;                               //guardem la dada al valor seguent on apunta PRxData
          if(RXByteCtr == 1){                                           //Queda nomÃ©s 1 dada?
